...
public class Client {
	...
	private final String IP= Constants.REMOTE_IP;
	...
	public synchronized void open(ISocketResponse mSocketResponseListener , String uuid)
	{
		this.mSocketResponseListener=mSocketResponseListener;
		this.uuid = uuid ;
		reconn();
				...
				while(state!=STATE_CLOSE)
				{
					...
					if(state==STATE_CONNECT_SUCCESS)
					{
						try {
							...
							if(uuid != null) {
								outStream.write(String.format("%010d" , uuid.getBytes().length).getBytes());
...
LogUtil.v(TAG,"Send ::End");
	...
	private class Rec implements Runnable
	{
	    ...
	    private byte[] Read(int length) throws Exception{
	        if(length > 4096) {
                LogUtil.e("Recv" , "Too big data package");
            }
            ...
            while((read=inStream.read(bodyBytes, offset, length))>0)
            {
                if(length-read==0)
                {
                	//LogUtil.v("Client" , new String(bodyBytes , "UTF-8"));
                    return bodyBytes ;
                }
                offset += read ;
                length -= read ;
...
LogUtil.v(TAG,"Rec :---------");
                        while(true) {
                            int len = Integer.parseInt(new String(Read(10 ), "UTF-8")) ;
                            byte[] bodyBytes = Read(len) ;
                            if(bodyBytes == null) {
